...
;(async () => {
	...
	for (let gamer = 0; true; gamer ^= 1) {
		 ...
		 */
		const gamerOperation = JSON.parse(await communication.question(''))[
			String(gamer)
		]
		if (
			!gamerOperation ||
...
const FieldItem tankItemTypes[sideCount][tankPerSide] = {{Blue0, Blue1}, {Red0, Red1}};
...
int main() {
	...
	for (int side = 0; side < TankGame::sideCount; side++) {
		...
		for (int i = 0; i < 3; i++) {
			...
			ss << " " << TankJudge::fieldBinary[i];
		}
		output["content"][int2str[side]] = ss.str();
	}

	output["command"] = "request";
	output["display"] = json(std::vector(std::begin(TankJudge::fieldBinary), std::end(TankJudge::fieldBinary))).dump();
